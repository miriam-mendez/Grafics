...
{
    cout << "[ShdowMapIndices plugin] C - Copy camera position to light position" << endl;
...
void ShadowMap::drawQuad(const Point& p0, const Point& p1, const Point& p2, const Point& p3)
{
    ...
    // 2. Draw
    g.glBindVertexArray(VAO_rect);
    g.glDrawElements(GL_TRIANGLE_STRIP, 4, GL_UNSIGNED_INT, 0); //numero de indices=4, enteros sin signo
    g.glBindVertexArray(0);
}
...
bool ShadowMap::paintGL()
{
    ...
    // Pass 2. Draw scene using shdowMapIndices
